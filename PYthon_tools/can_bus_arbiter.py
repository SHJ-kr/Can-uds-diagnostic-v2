...
class ArbitratedBus:
    ...
    def _arbiter_loop(self):
        # [실험했다가 되돌림] "대기 중인 게 1개뿐이면 경쟁 없음으로 보고 즉시
        # 내보내자"는 최적화를 시도했었다. 무경쟁 상황 지연은 0.77ms -> 0.37ms로
        # 잘 줄었지만, 진짜로 여러 스레드가 동시에 등록하는 상황에서 15번 중
        # 15번 다 ID 우선순위가 아니라 "누가 먼저 스레드 실행을 시작했는지"로
        # 승부가 갈리는 걸 실측으로 확인했다 - 파이썬 스레드 생성/스케줄 지연이
        # 수십us보다 훨씬 커서, "짧게 재확인"으로는 진짜 경쟁과 무경쟁을 구분할
        # 방법이 없었다. 이 기능의 목적 자체가 "ID 우선순위를 정확히 재현하는 것"
        # 이라 정확성을 못 버리므로, 매번 무조건 ARBITRATION_WINDOW를 기다리는
        # 원래 방식으로 되돌린다. 실제 하드웨어보다 프레임당 지연이 크다는 한계는
        # (모듈 docstring 참고) 남아있지만, 우선순위 자체는 항상 정확하다.
        #
        # [추가 확인된 한계] 새로 생성한 스레드 5~6개를 한 루프에서 한꺼번에
        # .start()하는 극단적인 테스트에서는, 그 스레드 생성/시작 자체에 걸리는
        # 시간이 이미 500us(ARBITRATION_WINDOW)를 넘어버려서 먼저 뜬 스레드가
        # 한 라운드 먼저 나가버리는(=ID 우선순위가 깨지는) 경우가 실측으로
        # 확인됐다(9/20 실패). 다만 이건 "매 메시지마다 스레드를 새로 만드는"
        # 비현실적인 부하 패턴에서만 그렇다 - 실제 BackgroundTrafficNode들은
        # 스레드를 한 번만 만들고 그 안에서 무작위 간격으로 반복 대기하므로,
        # 진짜로 여러 노드의 transmit() 호출이 같은 500us 안에 우연히 겹칠
        # 확률은 훨씬 낮다. 노드를 아주 많이(10개+) 추가하고 주기를 아주
        # 짧게(수 ms) 잡을수록 이 한계에 걸릴 가능성이 커진다는 점은 감안할 것.
        while not self._stop_flag.is_set():
            ...
            with self._cv:
                if not self._pending:
                    continue
                # 지금까지 모인 후보들 중 arbitration_id가 가장 낮은
                # (우선순위가 가장 높은) 걸 골라 이번 라운드의 승자로 정한다.
